...
public class ArvBinBusca<Chave extends Comparable<Chave>, Valor> 
{
    ...
    */
    ...
    /* Resposta Ex1 =  D;
     * 
     * Nessa busca, o 7 seria descendente do 8, o que é impossível para um arvore binaria de busca
     * que tem toda a subarvore esquerda menor do que o valor do nó
     * 
    */

    /* Complexidade Auxiliar
     *
     * Arvore Balanceada O(h) = log n
     * Arvore Desbalanceada O(h) = O(n);
     *
    */

    /* Metodo Ex2 */

    /* Compexidade O(n) * O(h) = O(n * h) */ 
    public void constroiArvore(int[] vetor){
        ...
        if (vetor == null)
            ...
            for (int i = 0; i < vetor.length; i++) 
                this.put((Chave)((Integer)vetor[i]), (Valor)((Integer)vetor[i]));
    }

    /* Metodo ex3 */

    /* Complexidade O(n) * O(h) = O(n * h)  */
    public void transformaArvBinBusca(Arvbin<Chave> arvoreBinaria){

        this.put(arvoreBinaria.retornaVal(), (Valor)arvoreBinaria.retornaVal());
    ...
    public boolean removeForaIntervalo(Chave chaveMin, Chave chaveMax){
        ...
        removeForaIntervalo(this.raiz, chaveMin, chaveMax);

        int tamanhoFinal = tamanho();

        return (tamanhoAnterior > tamanhoFinal);
    }

    /* Complexidade O(n) * delete - O(h) */
    ...
    /* Complexidade O(h) */
    private No obtemAncestralComum(No primeiro, No segundo){

        return obtemAncestralComum(this.raiz, primeiro, segundo);
    }

    private No obtemAncestralComum(No atual, No primeiro, No segundo) {

        if (atual == null || primeiro == null || segundo == null) 
            return null;

        if (primeiro.chave.compareTo(atual.chave) < 0 && segundo.chave.compareTo(atual.chave) < 0) 
            return obtemAncestralComum(atual.esq, primeiro, segundo);
        
        if (primeiro.chave.compareTo(atual.chave) > 0 && segundo.chave.compareTo(atual.chave) > 0) 
            return obtemAncestralComum(atual.dir, primeiro, segundo);
